...
from reworkd_platform.web.api.agent.ontology_reasoner import get_reasoner
...
class OpenAIAgentService(AgentService):
    ...
    async def analyze_task_agent(
        ...
        try:
            ...
            # 온톨로지 키워드 추론으로 폴백 도구 결정 (실패 시 기본값 사용)
            try:
                recommended = self.reasoner.recommend_tool(task.lower().split())
                return Analysis(
                    reasoning="Falling back to ontology-recommended tool.",
                    action=recommended,
                    arg=task,
                )
            except Exception:
    ...
    async def execute_task_agent(
        ...
        *,
        ...
        tool_name = analysis.action
        tool_class = get_tool_from_name(tool_name)

        # 도구를 사용할 수 없을 때 온톨로지 폴백 체인 적용
        if not tool_class.available():
            fallback_name = self.reasoner.get_fallback_tool(tool_name)
            if fallback_name:
                logger.info(
                    f"Tool '{tool_name}' unavailable; "
                    f"falling back to '{fallback_name}' (ontology)"
                )
                tool_class = get_tool_from_name(fallback_name)
    ...
    async def summarize_task_agent(
        ...
        *,
        ...
        self.model.model_name = summarize_model
        self.model.max_tokens = (
            self.reasoner.get_model_max_tokens(summarize_model) or 8000
        )
